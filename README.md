...
Congratulations! We have now successfully configured an account lockout policy in Active Directory using Group Policy. Lets Put it to the test by logging in to one of our domain users and see the account lockout policy in action.
...
- Once you open up Active Directory Users and Computers from within DC-1 you can simply right click onto mydomain.com and click on Find.
...
- Click on Account within the users properties.
...
- Now, go back to the Client-1 login and attempt to login to Client-1 with the user you just unlocked using the correct password and you should be able to log in just fine!
...
- When you search your user on DC-1 in the Find section of Active Directory Users and Computers, instead of double clicking on the user you can right click and from there you have the option to reset the users password as well as unlocking it if necessary.
- You will notice there are plently of other things we can do from there such as disbale the account, add them to a group, and more all from within Active Director Users and Computers on the domain controller.
